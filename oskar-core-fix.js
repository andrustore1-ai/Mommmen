...
/* ===== OSKAR RESTAURANT SIDEBAR GLOBAL RESTORE 2026-05-09 ===== */
(function(){
  'use strict';
  if(window.__OSKAR_RESTAURANT_SIDEBAR_RESTORE_20260509__) return;
  window.__OSKAR_RESTAURANT_SIDEBAR_RESTORE_20260509__ = true;

  const RESTAURANT_LINKS = [
    ['مطعم-الطاولات.html', 'الطاولات'],
    ['مطعم-الحجوزات.html', 'الحجوزات'],
    ['مطعم-المطبخ.html', 'المطبخ KDS'],
    ['مطعم-المنيو-الرقمي.html', 'المنيو الرقمي'],
    ['مطعم-الوصفات-والتكلفة.html', 'الوصفات والتكلفة'],
    ['مطعم-تحليلات-الأرباح.html', 'الأرباح والتحليلات'],
    ['مطعم-كاشير-المطعم.html', 'كاشير المطعم'],
    ['مطعم-مخزون-المطعم.html', 'مخزون المطعم']
  ];
  ...
  function isActive(href){ return currentFile() === href; }
  function esc(s){return String(s == null ? '' : s).replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}
  function restIcon(){
    return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3v8"/><path d="M10 3v8"/><path d="M8 3v18"/><path d="M14 3h4v18"/></svg>';
  }
  function simpleIcon(){
    return '<span class="sub-icon">🍽️</span>';
  }
  function ensureStyle(){
    if(document.getElementById('oskar-restaurant-sidebar-restore-style')) return;
    const s=document.createElement('style');
    s.id='oskar-restaurant-sidebar-restore-style';
    s.textContent = `
      #oskarRestaurantMenuGroup .submenu{display:block!important}
      #oskarRestaurantMenuGroup .menu-head{background:#e8f8ef!important;color:#064e3b!important}
      #oskarRestaurantMenuGroup .menu-head .mi{background:linear-gradient(135deg,#0f9f6e,#047857)!important}
      #oskarRestaurantMenuGroup .submenu a{display:flex!important;align-items:center;gap:8px}
      #oskarRestaurantMenuGroup .submenu a.active-link{background:#dff8eb!important;color:#047857!important;font-weight:950!important}
      .oskar-rest-nav-title{height:34px;display:flex;align-items:center;color:#064e3b;font-weight:950;font-size:13px;padding:0 6px;margin-top:4px}
    `;
    document.head.appendChild(s);
  }

  function buildDrawerGroup(){
    const submenu = RESTAURANT_LINKS.map(([href, label]) =>
      `<a href="${href}" class="${isActive(href)?'active-link':''}">${simpleIcon()}<span>${esc(label)}</span></a>`
    ).join('');
    return `<div class="menu-group open" id="oskarRestaurantMenuGroup" data-oskar-restaurant-menu="1">
      <button type="button" class="menu-head" onclick="this.parentElement.classList.toggle('open')">
        <span class="mi">${restIcon()}</span><b>المطعم</b><span class="arrow">‹</span>
      </button>
      <div class="submenu">${submenu}</div>
    </div>`;
  }

  function installDrawerMenu(){
    const menu = document.querySelector('.drawer .menu, .menu');
    if(!menu) return false;
    const existing = menu.querySelector('#oskarRestaurantMenuGroup');
    if(existing){
      existing.querySelectorAll('.submenu a').forEach(a=>a.classList.toggle('active-link', isActive(a.getAttribute('href')||'')));
      return true;
    }
    if(menu.querySelector('a[href="مطعم-الطاولات.html"], a[href*="مطعم-"]')) return true;
    const panel = menu.querySelector('#drawerBranchPanel, .drawer-branch-panel');
    const wrap = document.createElement('div');
    wrap.innerHTML = buildDrawerGroup();
    const node = wrap.firstElementChild;
    if(panel && panel.nextSibling) menu.insertBefore(node, panel.nextSibling);
    else if(panel) menu.appendChild(node);
    else menu.insertBefore(node, menu.firstChild);
    return true;
  }

  function buildRestaurantNavLink(href,label){
    return `<a class="${isActive(href)?'active':''}" href="${href}">${restIcon()}<span>${esc(label)}</span></a>`;
  }
  function installRestaurantNav(){
    const nav = document.getElementById('nav');
    if(!nav || !nav.classList.contains('nav')) return false;
    const isRestaurantShell = !!document.querySelector('.side-title') || /مطعم-/.test(currentFile()) || (window.REST_PAGE_KIND != null);
    if(!isRestaurantShell) return false;
    RESTAURANT_LINKS.forEach(([href,label])=>{
      if(!nav.querySelector(`a[href="${href}"]`)){
        const tmp=document.createElement('div');
        tmp.innerHTML=buildRestaurantNavLink(href,label);
        const back=[...nav.querySelectorAll('a')].find(a=>/العودة للنظام|لوحة-المتابعة/.test(a.textContent+(a.getAttribute('href')||'')));
        if(back) nav.insertBefore(tmp.firstElementChild, back); else nav.appendChild(tmp.firstElementChild);
      }
    });
    nav.querySelectorAll('a').forEach(a=>a.classList.toggle('active', isActive(a.getAttribute('href')||'')));
    return true;
  }
  ...
  function install(){
    ensureStyle();
    installDrawerMenu();
    installRestaurantNav();
  }

  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', install);
  else install();
  [50,150,350,800,1500,3000].forEach(ms => setTimeout(install, ms));
  try{
...
/* تحديث مركزي: إرجاع قسم المطعم في القائمة الجانبية بكل فروعه بدون حذف الأقسام الموجودة */
(function(){
  'use strict';
  if(window.__OSKAR_RESTAURANT_SIDEBAR_RESTORE_V2__) return;
  window.__OSKAR_RESTAURANT_SIDEBAR_RESTORE_V2__ = true;
  const links = [
    ['مطعم-الطاولات.html','الطاولات'],
    ['مطعم-الحجوزات.html','الحجوزات'],
    ['مطعم-المطبخ.html','المطبخ KDS'],
  ...
  function icon(){return '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 10h18"/><path d="M5 10l1.5 10h11L19 10"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg>';}
  function current(){try{return decodeURIComponent(location.pathname.split('/').pop()||'')}catch(e){return location.pathname.split('/').pop()||''}}
  function makeRestaurantBlock(){
    const curr=current();
    const inner=links.map(([href,label])=>`<a class="${curr===href?'active':''}" href="${href}"><span>${label}</span></a>`).join('');
    return `<div class="nav-group restaurant-sidebar-group" data-restaurant-sidebar="1" style="display:grid;gap:6px;margin:8px 0;">
      <div class="nav-group-title" style="display:flex;align-items:center;gap:8px;font-weight:950;color:#064e3b;padding:8px 10px;border-radius:14px;background:#ecfdf5;border:1px solid #d9e8e1;">${icon()}<span>المطعم</span></div>
      <div class="restaurant-sidebar-links" style="display:grid;gap:6px;padding-right:8px;">${inner}</div>
    </div>`;
  }
  function enhanceRestaurantNav(nav){
    if(!nav || nav.__restaurantEnhanced) return;
    nav.__restaurantEnhanced=true;
    const old=window.navHTML;
    if(typeof old==='function'){
      window.navHTML=function(){
        let html='';
        try{html=String(old.apply(this,arguments)||'')}catch(e){html=''}
        if(/data-restaurant-sidebar=|restaurant-sidebar-group|مطعم-الطاولات\.html|كاشير المطعم/.test(html)) return html;
        return makeRestaurantBlock()+html;
      };
    }
  }
  function restore(){
    const selectors=['#nav','.nav','nav.sidebar-menu','.sidebar-menu','aside nav'];
    const nav=selectors.map(s=>document.querySelector(s)).find(Boolean);
    if(!nav) return;
    enhanceRestaurantNav(nav);
    if(nav.querySelector('[data-restaurant-sidebar="1"]') || nav.innerHTML.includes('مطعم-الطاولات.html')) return;
    nav.insertAdjacentHTML('afterbegin', makeRestaurantBlock());
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', restore); else restore();
  setTimeout(restore,50); setTimeout(restore,300); setTimeout(restore,1000);
})();
